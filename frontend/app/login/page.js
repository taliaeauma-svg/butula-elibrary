...
export default function Home() {
  ...
  // Real "uploads this term" — count books added in the last 90 days
  const uploadsThisTerm = books.filter((b) => {
    if (!b.upload_date) return false;
    const uploaded = new Date(b.upload_date);
    const daysAgo = (Date.now() - uploaded.getTime()) / (1000 * 60 * 60 * 24);
    return daysAgo <= 90;
  }).length;
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <header className="border-b border-gray-200">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <a href="/" className="flex items-center gap-2">
            <div className="w-9 h-9 bg-[#6d1a2b] rounded-md flex items-center justify-center text-white text-sm font-bold">
              📖
            </div>
            <div>
          ...
          <nav className="hidden md:flex items-center gap-6 text-sm text-gray-600">
            <a href="/" className="hover:text-[#6d1a2b]">Library</a>
            <a href="#library" className="hover:text-[#6d1a2b]">Reading List</a>
            <a href="#categories" className="hover:text-[#6d1a2b]">Departments</a>
        ...
        <div className="grid md:grid-cols-3 gap-10 items-start">
          <div className="md:col-span-2">
            ...
            <h1 className="text-4xl sm:text-5xl font-serif text-gray-900 leading-tight mb-4">
              The whole school library,<br />
              <span className="italic text-[#6d1a2b]">in one shelf.</span>
            </h1>
            <p className="text-gray-500 mb-6 max-w-lg">
              Browse textbooks, past papers, CATs and teacher notes across
              every department. Read online or download for revision —
              anywhere, any time.
            </p>
              ...
              <div className="flex flex-wrap items-center gap-2 mt-3 text-xs text-gray-400">
                ...
                {categories.slice(0, 4).map((cat) => (
                  <button
                    ...
                    className="underline hover:text-[#6d1a2b]"
                  >
                    {cat.name}
                  </button>
                ))}
            ...
            {user && role === "admin" && (
              <button
                ...
                className="mt-4 text-sm text-[#6d1a2b] font-medium underline block"
              >
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div className="border border-gray-200 rounded-lg p-4">
                ...
                <div className="text-xs text-gray-400">across {categories.length} departments</div>
              </div>
              <div className="border border-gray-200 rounded-lg p-4">
                <div className="text-xs text-gray-400 uppercase tracking-wide">
                  Uploads This Term
                ...
                <div className="text-xs text-gray-400">new & updated</div>
              </div>
            </div>
            ...
            {!user && (
              <a
                href="/login"
                className="block bg-gray-50 border border-gray-200 rounded-lg p-4 hover:border-[#6d1a2b]/40 transition"
              >
                <div className="text-sm font-medium text-gray-700">
                  Sign in to see your shelf
                </div>
                <div className="text-xs text-gray-400 mt-1">
                  Personal stats appear here once logged in.
                </div>
              </a>
            )}
          </div>
      ...
      {/* Footer */}
      <footer id="help" className="border-t border-gray-200 bg-gray-50">
        <div className="max-w-6xl mx-auto px-6 py-10 grid sm:grid-cols-3 gap-8">
          <div>
            <div className="font-serif font-bold text-gray-800 mb-2">
              Butula E-Library
            </div>
            <p className="text-sm text-gray-500">
              Fostering a culture of curiosity and learning. Providing
              students with curated digital resources to support academic
              excellence.
            </p>
          </div>
          <div>
            ...
            <ul className="text-sm text-gray-500 space-y-1">
              <li><a href="#library" className="hover:text-[#6d1a2b]">Library Catalog</a></li>
              <li><a href="#" className="hover:text-[#6d1a2b]">Library Rules</a></li>
            </ul>
          </div>
          <div>
            <div className="text-xs font-semibold text-gray-700 uppercase tracking-wide mb-2">
            ...
            <ul className="text-sm text-gray-500 space-y-1">
              <li><a href="#" className="hover:text-[#6d1a2b]">Contact librarian</a></li>
              <li><a href="#" className="hover:text-[#6d1a2b]">Help & FAQ</a></li>
            </ul>
          </div>
        </div>
        <div className="text-center text-xs text-gray-400 py-4 border-t border-gray-200">
          © 2026 Butula E-Library. All rights reserved.
        </div>
      </footer>
    </div>
  );
